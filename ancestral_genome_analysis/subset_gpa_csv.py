...
def subsample_panaroo_rtab(gpas, isol_info, tree, out_table):
    ...
    t = pd.read_csv(gpas, index_col = 0)
    t.columns = [i.replace(".spades", "") for i in t.columns]
    t = t.loc[:, ['Non-unique Gene name', 'Annotation'] + names.panaroo_id.to_list()]
    t.columns = ['Non-unique Gene name', 'Annotation'] + names.loc[:, "sample_name"].to_list()
    t.to_csv(out_table)
